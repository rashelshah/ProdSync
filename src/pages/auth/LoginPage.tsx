...
import { Eye, EyeOff, Search, Bell, User, LogOut, Command, Package, Film, Star, CheckCircle, XCircle } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: 'Welcome back to ProdSync!', // Changed RentPro to ProdSync
      });
      navigate('/dashboard');
    }
    setIsLoading(false);
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) {
      ...
      toast({
        title: 'Login successful',
        description: 'Welcome to ProdSync!', // Changed RentPro to ProdSync
      });
      navigate('/dashboard');
    }
    setIsLoading(false);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center justify-center mb-8">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white text-black rounded-xl flex items-center justify-center"> {/* Changed w-12 h-12 bg-gradient-primary shadow-primary to w-10 h-10 bg-white text-black */}
              <Film className="w-6 h-6" /> {/* Changed Package to Film */}
            </div>
            <span className="text-2xl font-bold text-foreground">ProdSync</span> {/* Changed RentPro to ProdSync */}
          </div>
        </div>
